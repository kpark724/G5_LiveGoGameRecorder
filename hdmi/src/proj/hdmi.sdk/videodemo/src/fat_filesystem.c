...
s32 init_fat_filesystem() {
	...
	sd_debug_print_hex(bsraw, SD_INIT_SEC_SIZE, 16);
...
static s32 fat_strncmp(u8 * str1, u8 * str2, u32 len) {
	int i;
...
static void print_root_dir(ROOT_DIR_ENTRY_T * rde) {
	int i;
	u16 tc, dc;
...
static s32 allocate_new_cluster(FATTIE * fd) { //function should be made static later
	s32 i, j, status;
	...
	if(found) {
		for(j = 0; j < FS_filesystem.bootsector.FAT_count;++j) {
			status = sd_write(FAT_SECTOR(fd->clusters[fd->num_clusters]) + FS_filesystem.fat_sector + j * FS_filesystem.bootsector.sectors_per_FAT,
			...
			if(fd->num_clusters != 0) {
				status = sd_write(FAT_SECTOR(fd->clusters[fd->num_clusters - 1]) + FS_filesystem.fat_sector + j * FS_filesystem.bootsector.sectors_per_FAT,
									FAT_OFFSET(fd->clusters[fd->num_clusters - 1]),
									(u8 *)(&(fd->clusters[fd->num_clusters])), (u32)2);
			}
...
FATTIE * fat_open(u8 * filename) {
	...
	int i, j;
	FATTIE * fd = (FATTIE *)malloc(sizeof(fd));
	...
	fd->dir_entry_sec = i + FS_filesystem.root_dir_sector;
	fd->dir_entry_index = j;
	...
	fd->num_clusters = 0;	//initial number of clusters (before reading dir entry)
	...
	if(dir_entry->filename[0] == 0x0) {
		...
		dir_entry->start_cluster = fd->clusters[0];
...
void fat_seek(FATTIE * fd, u32 byte) {
	if(fd == NULL)
		return;
...
s32 fat_read(FATTIE * fd, u8 * buffer, u32 length) {
	s32 len = 0;
	s32 i;
	s32 status;
...
s32 fat_write(FATTIE * fd, u8 * buffer, u32 length) {
	s32 i;
	s32 status;
	s32 len = 0;
	//u8 FAT_sector[SD_INIT_SEC_SIZE];
	//ROOT_DIR_ENTRY_T * dir_entry;
	//u8 root_buffer[SD_INIT_SEC_SIZE];
	...
	while(1) {
		for(i = 0; i < length && fd->current_offset + i < SD_INIT_SEC_SIZE && fd->current_cluster < fd->max_clusters; ++i) {
	...
	/*
	status = sd_read(fd->dir_entry_sec, 0, root_buffer, SD_INIT_SEC_SIZE);
	dir_entry = (ROOT_DIR_ENTRY_T *)(root_buffer + fd->dir_entry_index * 32);

	if(dir_entry->file_size != fd->file_size) {
		dir_entry->file_size = fd->file_size;
		status = sd_write(fd->dir_entry_sec, 0, root_buffer, SD_INIT_SEC_SIZE);
...
void fat_close(FATTIE * fd) {
	s32 status;
	u8 buffer[SD_INIT_SEC_SIZE];
	ROOT_DIR_ENTRY_T * dir_entry;
	...
	status = sd_read(fd->dir_entry_sec, 0, buffer, SD_INIT_SEC_SIZE);
	dir_entry = (ROOT_DIR_ENTRY_T *)(buffer + fd->dir_entry_index * 32);

	if(dir_entry->file_size != fd->file_size) {
		dir_entry->file_size = fd->file_size;
		status = sd_write(fd->dir_entry_sec, 0, buffer, SD_INIT_SEC_SIZE);
